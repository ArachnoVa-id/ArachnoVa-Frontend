import { FaHandshake } from "react-icons/fa6";

const Container = ({ Icon, Title, Description, State, Clicked }) => {
	const style = State
		? "border-[0.030rem] border-border bg-white"
		: "bg-transparent";
	return (
		<div
			className={`group lg:w-[47.4rem] lg:h-[9.9rem] w-[50rem] h-[41.86rem] flex flex-row lg:p-[1.8rem] gap-x-[1.5rem] rounded-[1.05rem] lg:cursor-pointer z-50 ${style}}`}
			onClick={Clicked}
		>
			<div className="w-fit h-fit lg:flex hidden">{Icon}</div>
			<div className="flex flex-col gap-y-[0.68rem]">
				<div className="w-fit h-fit flex lg:hidden">{Icon}</div>
				<p className="lg:group-hover:bg-clip-text lg:group-hover:text-transparent lg:group-hover:bg-gradient-to-r lg:group-hover:from-[#1AB0C8] lg:group-hover:via-[#84D4E1] lg:group-hover:to-[#179FB5] transition-all duration-500 ease-in-out font-SourceSansProBold lg:text-[2.1rem] text-[6.7rem]/[1.64rem] text-neutral-g lg:leading-[2.25rem]">
					{Title}
				</p>
				<p className="font-SourceSansProSemibold lg:text-[1.2rem] text-[5.36rem] lg:leading-normal leading-[6.7rem] text-[#64748B]">
					{Description}
				</p>
			</div>
		</div>
	);
};

export default Container;
