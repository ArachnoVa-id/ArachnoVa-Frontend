...
export default function ProjectCardGrid({ projects, autoOpenId, onAutoOpenDone, cardRefs: externalRefs }) {
  ...
  useEffect(() => {
    if (autoOpenId && cardRefs.current[autoOpenId]) {
      const card = cardRefs.current[autoOpenId];
      const descImg = card.querySelector(".desktop-card-img");
      const mobImg = card.querySelector(".mobile-card-img");
      setOriginRect({
        el: card,
        desktopRect: descImg?.getBoundingClientRect?.() || null,
        mobileRect: mobImg?.getBoundingClientRect?.() || null,
      });
              ...
              <div className="relative w-full aspect-[824.28/426.9] bg-gray-50">
                ...
                {hasMobile && (
                  <img src={project.imageMobile || project.mobileImages[0]} alt=""
                    className="mobile-card-img absolute w-[22%] aspect-[245/485] rounded-[0.2rem] shadow-lg -bottom-[2%] left-[4%]"
                    draggable="false" loading="lazy" />
                )}
              </div>
