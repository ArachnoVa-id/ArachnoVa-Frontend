...
export default function MarqueeCarousel({ items, direction = "left", onItemClick }) {
  ...
  return (
    <div className="overflow-hidden w-full">
      <div ref={containerRef} className="flex gap-[0.72rem] will-change-transform" style={{ width: "max-content" }}>
        {doubled.map((item, i) => (
          <div
            key={i}
            onClick={() => onItemClick?.(item.projectId)}
            className="relative group flex-shrink-0 lg:w-[20.16rem] w-[50rem] aspect-[4/3] rounded-lg overflow-hidden border border-border shadow-sm cursor-pointer"
          >
            <img
              src={item.src}
              alt=""
